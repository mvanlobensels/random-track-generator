...
from .utils import closest_node, clockwise_sort, curvature, arc_length, transformation_matrix
...
class TrackGenerator:
    ...
    def __init__(self, 
        ...
        self.min_bound = min_bound                                          # [m]
        self.max_bound = max_bound                                          # [m]
        self.bounding_box = np.array([self.min_bound, self.max_bound] * 2)  # [x_min, x_max, y_min, y_max]

        self.mode = Mode[mode.upper()] if isinstance(mode, str) else Mode(mode)

        # Track parameters
        self.track_width = 3.                   # [m]
        self.cone_spacing = 5.                  # [m]
        self.length_start_area = 6.             # [m]
        self.curvature_threshold = 1. / 3.75    # [m^-1]
        self.straight_threshold = 1. / 100.     # [m^-1]
    ...
    def create_track(self, seed: int | None = None) -> Track:
        ...
        while True:
            ...
            if self.mode == Mode.EXPAND:
                ...
                end = (random_point[0] + 1./2. * self.max_bound * np.cos(random_heading), random_point[1] + 1./2. * self.max_bound * np.sin(random_heading))
        ...
        cone_spacing_left = np.linspace(0, track_left.length, np.ceil(track_left.length / self.track_width).astype(int) + 1)[:-1]
        cone_spacing_right= np.linspace(0, track_right.length, np.ceil(track_right.length / self.track_width).astype(int) + 1)[:-1]
        ...
        straight_threshold = self.straight_threshold if abs_curvature.min() < self.straight_threshold else abs_curvature.min() + 0.1
        ...
        length_start_area = self.length_start_area if length_straights.max() > self.length_start_area else length_straights.max()
        try:
